// ═══════════════════════════════════════════════════════════════════════════
// GPUMemoryBuffer - RAII класс для управления GPU памятью CPU ↔ GPU
// Использует pinned memory для оптимального DMA трансфера
// ═══════════════════════════════════════════════════════════════════════════
...
enum class MemoryType {
    GPU_WRITE_ONLY,      // Kernel пишет, CPU читает
    GPU_READ_ONLY,       // CPU пишет, kernel читает
    GPU_READ_WRITE,      // Оба читают и пишут
    PINNED_HOST,         // Pinned memory на хосте (быстрый DMA трансфер)
};

// ═════════════════════════════════════════════════════════════════════════════
// GPUMemoryBuffer - основной класс для управления памятью
// ═════════════════════════════════════════════════════════════════════════════

class GPUMemoryBuffer {
public:
    // ═══════════════════════════════════════════════════════════════════════
    // КОНСТРУКТОР - выделяет память GPU + pinned memory на хосте
    // ═══════════════════════════════════════════════════════════════════════
        ...
        is_gpu_dirty_(false)
    {
        ...
        // Выделить CPU pinned память для DMA трансфера
        AllocatePinnedHostBuffer();
        
        std::cout << "✅ GPUMemoryBuffer created: " 
                  << (GetTotalBytes() / (1024.0 * 1024.0)) << " MB (GPU + Pinned Host)\n";
    }

    // ═══════════════════════════════════════════════════════════════════════
    // ДЕСТРУКТОР - освобождает GPU + HOST память (RAII)
    // ═══════════════════════════════════════════════════════════════════════
    ...
    ~GPUMemoryBuffer() noexcept {
        try {
            ...
            std::cerr << "⚠️  Exception in ~GPUMemoryBuffer: " << e.what() << "\n";
        }
    }
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // PUBLIC API: Трансфер GPU → CPU (с pinned memory оптимизацией)
    // ═══════════════════════════════════════════════════════════════════════
    ...
    std::vector<std::complex<float>> ReadFromGPU() {
        ...
        std::cout << "📥 Transferring data GPU → CPU (" 
        ...
        // Дождаться завершения всех операций на GPU
        clFinish(queue_);

        // Шаг 2: Pinned Host Memory → CPU Vector (очень быстро, т.к. pinned)
        ...
        is_gpu_dirty_ = false;
        std::cout << "✅ Data transfer GPU → CPU complete\n";
        return result;
    }

    // ═══════════════════════════════════════════════════════════════════════
    // PUBLIC API: Трансфер CPU → GPU (с pinned memory оптимизацией)
    // ═══════════════════════════════════════════════════════════════════════
    ...
    void WriteToGPU(const std::vector<std::complex<float>>& data) {
        ...
        std::cout << "📤 Transferring data CPU → GPU (" 
                  << (GetTotalBytes() / (1024.0 * 1024.0)) << " MB)...\n";

        // Шаг 1: CPU Vector → Pinned Host Memory (очень быстро)
        std::memcpy(
            cpu_pinned_buffer_.get(),
            data.data(),
            GetTotalBytes()
        ...
        // Шаг 2: Pinned Host Memory → GPU (быстрый DMA)
        ...
        is_gpu_dirty_ = true;
        std::cout << "✅ Data transfer CPU → GPU complete\n";
    }

    // ═══════════════════════════════════════════════════════════════════════
    // PUBLIC API: Частичное чтение (первые N элементов)
    // ═══════════════════════════════════════════════════════════════════════
    ...
    std::vector<std::complex<float>> ReadPartial(size_t num_samples) {
        ...
        std::cout << "📥 Partial read GPU → CPU (" << num_samples << " samples)...\n";

        // Читаем только нужное количество элементов
        ...
        std::cout << "✅ Partial read complete\n";
        return result;
    }

    // ═══════════════════════════════════════════════════════════════════════
    // PUBLIC API: Получить GPU буфер для kernel
    // ═══════════════════════════════════════════════════════════════════════
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // PUBLIC API: Получить CPU pinned буфер
    // ═══════════════════════════════════════════════════════════════════════
...
private:
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // PRIVATE: Выделение pinned host памяти
    // ═══════════════════════════════════════════════════════════════════════
    ...
    void AllocatePinnedHostBuffer() {
        ...
        // Создать pinned memory buffer для оптимального DMA трансфера
        ...
        // Отобразить pinned memory в адресное пространство CPU
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // PRIVATE: Освобождение GPU памяти
    // ═══════════════════════════════════════════════════════════════════════
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // PRIVATE: Освобождение pinned host памяти
    // ═══════════════════════════════════════════════════════════════════════
