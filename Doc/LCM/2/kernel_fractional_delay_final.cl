 ...
 * - 256 антен × 1.3М комплексных отсчётов
 * 
 * ВЫХОДНЫЕ ДАННЫЕ:
 ...
 * @param num_samples Размер сигнала (1,300,000)
 ...
 * 
 * РАЗЛОЖЕНИЕ РАБОТ:
 * - global_id = 0..255      → sample=0, beam=0..255 (все антены отсчёта 0)
 * - global_id = 256..511    → sample=1, beam=0..255 (все антены отсчёта 1)
 * - global_id = 512..767    → sample=2, beam=0..255 (все антены отсчёта 2)
 * ...
 * 
 * РЕЗУЛЬТАТ:
 * - Work items 0-255 работают ПАРАЛЛЕЛЬНО (один warp)
 * - Memory coalescing: ИДЕАЛЬНЫЙ (256 антен → 1 обращение к памяти)
 * - Bandwidth: ~90% от максимума
 * 
 * ПАРАМЕТРЫ:
 * @param input Буфер входных данных [num_samples * num_beams]
 *              Формат: float2 (complex), кол-во элементов = 1.3М × 256 = 332.8М
 ...
 * @param delay_params Параметры задержки для каждой антены [256]
 *                     DelayParams: {delay_integer, lagrange_row}
 * @param num_beams Количество лучей (антен) = 256
 * @param num_samples Количество отсчётов на луч = 1,300,000
 */
__kernel void fractional_delay_optimized(
    ...
    __global const DelayParams* delay_params,      // [num_beams] = [256]
    ...
    uint sample_id = global_id / num_beams;   // Какой отсчёт (0..1.3М)
    uint beam_id = global_id % num_beams;     // Какая антена (0..255)
    ...
    #pragma unroll
    for (int k = 0; k < LAGRANGE_COLS; k++) {
        // Адресация ПОПЕРЕЧНАЯ: все 256 антен читают разные антены одного отсчёта
...
/**
 * @brief Вариант с 2D work group (X=256 антен, Y=1 отсчёт)
 * 
...
__kernel void fractional_delay_2d(
    ...
    // 2D индексирование
    uint beam_id = get_global_id(0);       // 0..255 (антена)
    uint sample_id = get_global_id(1);     // 0..1.3М (отсчёт)
...
2. ЗАПУСК ЯДРА (для каждого блока данных):
   ...
   size_t local_size = 256;              // 256 антен в группе
...
Input format (после чтения):
┌─────────────────────────────────────────────────┐
│ [antenna_0_sample_0, antenna_1_sample_0, ..., antenna_255_sample_0,
│  antenna_0_sample_1, antenna_1_sample_1, ..., antenna_255_sample_1,
│  ...
│  antenna_0_sample_1299999, antenna_1_sample_1299999, ...] │
└─────────────────────────────────────────────────┘
       ↑
